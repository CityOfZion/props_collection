...
export const initWalletConnect = async () => {
  ...
  wcSdk = await WcSdk.init({
    ...
    metadata: {
      ...
      icons: [new URL('./props_icon.svg', import.meta.url).href],
    },
  })
...
function changeInvokeButton(formElement: Element | null, buttonStage: ButtonStage) {
  ...
  if (buttonStage === ButtonStage.WaitWallet || buttonStage === ButtonStage.Approved) {
    ...
    const approvedElement = `
      <img src="/checkmark_icon.svg" class="position-absolute ms-4 top-50 start-0 translate-middle" style="width:1.5rem; height:1.5rem;" role="status">
